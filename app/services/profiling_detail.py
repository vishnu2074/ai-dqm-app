...
from app.services.dq_scores import _load_dataframe_for_dataset, _fmt_ist, run_dq_scoring
...
_DATE_COL_RE = re.compile(r"(date|time|dt|day|month|year|created|updated|timestamp|_at|_on|since|until|start|end|birth|dob|expir)", re.I)
...
_SENSITIVE_PATTERNS = {
    "PII": re.compile(r"(email|phone|mobile|cell|ssn|social.*security|passport|national.*id|drivers.*license|dob|birth.*date|first.*name|last.*name|full.*name)", re.I),
    "Financial": re.compile(r"(salary|income|bank.*account|credit.*card|balance|amount|price|cost|revenue|payment|purchase|transaction)", re.I),
    "Health": re.compile(r"(patient|diagnosis|medical|health|prescription|treatment|disease|symptom)", re.I),
    "Authentication": re.compile(r"(password|secret|token|api.*key|credential|auth)", re.I),
    "Location": re.compile(r"(address|latitude|longitude|gps|coordinates|zip.*code|postal|city|country)", re.I),
}
...
def _save_ai_summary_to_db(run_id: int, summary: str):
    ...
    try:
        ...
        with engine.connect() as conn:
            conn.execute(sa_text("UPDATE profiling_runs SET ai_summary = :summary WHERE id = :id"), {"summary": summary, "id": run_id})
            conn.commit()
...
def _save_sensitivity_to_db(profiling_run_id: int, column_name: str, sensitivity: str):
    try:
        ...
        with engine.connect() as conn:
            ...
            # REAL AUDIT LOG: Write a real entry for this classification
            try:
                conn.execute(sa_text("""
                    INSERT INTO governance_audit_log (action, user_id, created_at, details)
                    VALUES ('SENSITIVITY_LABEL_APPLIED', 'system', datetime('now'), :details)
                """), {"details": f"Column '{column_name}' classified as '{sensitivity}' (run {profiling_run_id})"})
            except Exception as audit_err:
                print(f"[profiling] Audit log write skipped: {audit_err}")
...
def _histogram(series: pd.Series, bins: int = 20) -> list:
    try:
        ...
        return [{"bin_start": round(float(edges[i]),4), "bin_end": round(float(edges[i+1]),4), "count": int(counts[i])} for i in range(len(counts))]
...
def _ai_dataset_description(ds_name: str, total_rows: int, columns: list[dict], summary: dict) -> str:
    # Priority 1: Use centralized LLM client (correct Bearer auth)
    try:
        ...
        if client:
            ...
            prompt = (
                ...
                f"Types: {summary.get('numericColumns',0)} numeric, {summary.get('categoricalColumns',0)} categorical, {summary.get('datetimeColumns',0)} datetime, {summary.get('stringColumns',0)} string\n"
    ...
    if _KEY and _ENDPOINT:
        try:
            ...
            for c in columns[:20]:
                ...
                col_lines.append(f"  {c['columnName']} ({ctype}, semantic={sem}, nulls={c.get('nullPct',0):.1f}%, unique={c.get('distinctPct',0):.0f}%)")
            prompt = (
                f"You are a data catalog expert. Describe dataset '{ds_name}' ({total_rows:,} rows, {len(columns)} columns). "
            ...
            model_name = os.getenv("AZURE_OPENAI_DEPLOYMENT", os.getenv("AZURE_OPENAI_MODEL", "Llama-3.3-70B-Instruct"))
            _t1 = _time.time()
            r = _http.post(f"{_ENDPOINT}/chat/completions", headers={"Content-Type": "application/json", "Authorization": f"Bearer {_KEY}"},
...
def _fallback_description(ds_name: str, total_rows: int, columns: list[dict], summary: dict) -> str:
    ...
    return (f"This dataset, '{ds_name}', contains {total_rows:,} records across {n_cols} columns representing {domain}. "
            f"The schema includes {num_c} numeric measure(s), {cat_c} categorical attribute(s), {dt_c} date/time field(s), and {str_c} text/string column(s).\n\n"
            f"Overall data completeness is {comp:.1f}% with an average column uniqueness of {uniq:.1f}%. Key columns include: {key_cols}.\n\n"
            f"• Use for analytical reporting and KPI dashboards\n• Integrate with related datasets via shared identifier columns\n• Apply DQ rules to enforce data validity and completeness standards")
...
def _entity_profile(col: str, ctype: str, series: pd.Series) -> dict:
    ...
    if sem_type == "Text":
        ...
        elif re.search(r"(amount|price|cost|revenue|salary|income|fee|tax|total|balance)", col_l): sem_type, role = "Amount", "Measure"
    ...
    return {"semanticType":sem_type,"role":role,"entityGroup":group,"pkScore":round(pk,2),"fkScore":round(fk,2),"isNullable":bool(series.isna().any())}

def _pattern_profile(non_null: pd.Series) -> dict:
    if len(non_null) == 0:
        return {"dominantPattern":None,"consistencyPct":None,"patternsBreakdown":[],"mixedFormatFlag":False,"invalidCount":0,"invalidPct":0.0,"shapeSummary":[]}
    ...
    shape_summary = sorted([{"shape":sh,"count":len(ex),"example":ex[0]} for sh,ex in shapes.items()], key=lambda x:-x["count"])[:5]
    return {"dominantPattern":dominant["pattern"] if dominant else None,"consistencyPct":dominant["pct"] if dominant else None,
            "patternsBreakdown":breakdown,"mixedFormatFlag":mixed,"invalidCount":invalid,"invalidPct":round(invalid/max(len(non_null),1)*100,2),"shapeSummary":shape_summary}
...
def _profile_column(col: str, ctype: str, series: pd.Series, total_rows: int) -> dict:
    ...
    base: dict = {"columnName": col, "typeClass": ctype, "columnType": _infer_column_type(col, series, ctype),
                  "count": count, "nullCount": null_count, "nullPct": null_pct, "distinctCount": distinct,
                  "distinctPct": _safe(distinct/max(count,1)*100), "duplicateCount": max(0, count-distinct), "outliers": []}
    if ctype == "NUMERIC":
        num = pd.to_numeric(non_null, errors="coerce").dropna()
        if len(num) >= 2:
            q25,q75 = float(np.percentile(num,25)), float(np.percentile(num,75))
            base.update({"min":_safe(num.min()),"max":_safe(num.max()),"mean":_safe(num.mean()),"median":_safe(num.median()),
                "std":_safe(num.std()),"variance":_safe(num.var()),"sum":_safe(num.sum()),"range":_safe(float(num.max())-float(num.min())),
                "p05":_safe(np.percentile(num,5)),"p25":_safe(q25),"p50":_safe(np.percentile(num,50)),"p75":_safe(q75),
                "p95":_safe(np.percentile(num,95)),"p99":_safe(np.percentile(num,99)),"iqr":_safe(q75-q25),
                "skewness":_safe(float(num.skew())),"kurtosis":_safe(float(num.kurtosis())),"histogram":_histogram(num)})
        ...
        base.update({"entropy":_safe(-float((probs*np.log2(probs+1e-12)).sum())) if len(probs) else None,
            "mostFrequentValue":str(vc.index[0]) if len(vc) else None,"mostFrequentCount":int(vc.iloc[0]) if len(vc) else None,
            "rareValueCount":int((vc<total_cat*RARE_THRESHOLD_PCT/100).sum()),
            "topKValues":[{"value":str(v),"count":int(c),"pct":round(c/max(total_cat,1)*100,2)} for v,c in zip(vc.head(TOP_K).index,vc.head(TOP_K).values)]})
        ...
        try:
            ...
            if len(dates) >= 2:
                ...
                base.update({"minDate":str(dates.min().date()),"maxDate":str(dates.max().date()),
                    "dateRangeDays":int((dates.max()-dates.min()).days),"avgGapDays":_safe(gaps.mean()),"maxGapDays":_safe(gaps.max()),
                    "futureDateCount":int((dates>today).sum()),"nullDateCount":null_count})
                base["outliers"] = _date_outliers(dates, today)
        except Exception: pass
    elif ctype == "STRING":
        lengths = non_null.astype(str).str.len()
        base.update({"minLength":_safe(lengths.min()) if len(lengths) else None,"maxLength":_safe(lengths.max()) if len(lengths) else None,
            "avgLength":_safe(lengths.mean()) if len(lengths) else None,"lengthStd":_safe(lengths.std()) if len(lengths) else None,
...
def _dataset_summary(columns: list, total_rows: int) -> dict:
    n = max(len(columns), 1)
    return {"totalRows": total_rows, "totalColumns": len(columns),
...
def _build_profile_payload(run: ProfilingRun, df: pd.DataFrame, total_runs: int, ds_name: str) -> dict:
    ...
    return {"status": "ok", "runId": run.id, "runTimestamp": _fmt_ist(run.timestamp),
            "runType": "FULL" if run.is_full_scan else "INCREMENTAL", "deltaRows": run.delta_rows or 0,
            "totalRuns": total_runs, "datasetName": ds_name, "totalRows": total_rows,
            "totalColumns": len(df.columns), "columns": columns, "datasetSummary": summary, "description": description}
...
def run_detail_profiling(db: Session, dataset_id: int) -> dict:
    ...
    for col_data in payload.get("columns", []):
        ...
        col_desc = f"Column '{col_name}' is of type {type_class} (semantic: {sem_type}). Completeness: {100 - null_pct:.1f}%, Uniqueness: {distinct_pct:.0f}%."
        if null_pct > 0: col_desc += f" Contains {col_data.get('nullCount', 0)} null values."
        if col_data.get("distinctCount", 0) > 0: col_desc += f" Has {col_data.get('distinctCount', 0)} distinct values."
        _save_ai_description_to_db(run.id, col_name, col_desc)
    return payload

def get_detail_profile(db: Session, dataset_id: int) -> dict:
    run = (db.query(ProfilingRun).filter(ProfilingRun.dataset_id==dataset_id, ProfilingRun.status=="COMPLETED").order_by(ProfilingRun.id.desc()).first())
