...
class Dataset(Base):
    ...
    profiling_runs = relationship(
        "ProfilingRun",
        back_populates="dataset",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )
    profiling_baselines = relationship(
        "ProfilingBaseline",
        back_populates="dataset",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )
    schema_history_entries = relationship(
        "SchemaHistory",
        back_populates="dataset",
        cascade="all, delete-orphan",
        passive_deletes=True,
    ...
    dq_rule_change_logs = relationship(
        "DQRuleChangeLog",
        back_populates="dataset",
        cascade="all, delete-orphan",
        passive_deletes=True,
...
class ProfilingRun(Base):
    ...
    drift_records = relationship(
        "DriftRecord",
        foreign_keys="DriftRecord.profiling_run_id",
        back_populates="profiling_run",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )
    compared_drift_records = relationship(
        "DriftRecord",
        foreign_keys="DriftRecord.comparison_run_id",
...
class QualityCheck(Base):
    ...
    resolved_by_rule_id = Column(
        Integer, ForeignKey("dq_rules.id", ondelete="SET NULL"), nullable=True
    )
...
class DriftRecord(Base):
    ...
    comparison_run_id = Column(
        Integer, ForeignKey("profiling_runs.id", ondelete="CASCADE"), nullable=True
    )
 
    profiling_run = relationship(
        "ProfilingRun",
        foreign_keys=[profiling_run_id],
        back_populates="drift_records",
    )
    comparison_run = relationship(
        "ProfilingRun",
        foreign_keys=[comparison_run_id],
        back_populates="compared_drift_records",
    )
...
class DQRuleRunResult(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    run_id = Column(
        Integer, ForeignKey("dq_rule_runs.id", ondelete="CASCADE"), nullable=False, index=True
    )
