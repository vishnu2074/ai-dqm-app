...
def M(id_, label, value, unit, status, formula, details=None, **extra) -> dict:
    ...
    return {
        ...
        "status": status, "formula": formula, "details": details or {}, **extra,
    }
...
def _introspect(conn) -> dict:
    """
    Detect which optional columns actually exist in key tables and 
    dynamically map status values to logical states.
    """
    ...
    # ── 1. profiling_runs status mapping ──────────────────────────────────────
    pr_statuses = set()
        ...
        if not drift_has_severity:
            for candidate in ("magnitude", "change_value", "delta", "value", "score"):
                if candidate in dr_cols:
                    drift_magnitude_col = candidate
                    break

    # ── 4. temporal_checks context column ─────────────────────────────────────
    tc_statuses = set()
    tc_context_col = None
    if _table_exists(conn, "temporal_checks"):
        ...
        for candidate in ("explanation", "root_cause", "analysis", "notes", "details", "description"):
            if candidate in tc_cols:
                tc_context_col = candidate
                break

    # ── 5. profiling_runs timestamp column ────────────────────────────────────
    pr_timestamp_col = None
    for candidate in ("created_at", "started_at"):
        if candidate in pr_cols:
            pr_timestamp_col = candidate
            break
    ...
    return {
        ...
        "pr_completed_filter": pr_completed_filter,
        "pr_failed_filter": pr_failed_filter,
        "pr_timestamp_col": pr_timestamp_col,
        "pr_statuses": pr_statuses,
        
        "tc_has_explanation": tc_context_col is not None,
        "tc_context_col": tc_context_col,
        ...
        "drift_has_severity": drift_has_severity,
        "drift_magnitude_col": drift_magnitude_col,
        
        "dq_active_filter": dq_active_filter,
...
def _duration_stats(conn, schema, dataset_id=None) -> dict:
    ...
    if pr["pr_has_duration_ms"]:
        row = _one(conn,
            ...
            f"WHERE {comp_filter} AND duration_ms IS NOT NULL {ds_filter}", params)
        if row and row[1]:
    ...
    if pr["pr_has_started_at"] and pr["pr_has_completed_at"]:
        rows = _all(conn,
            ...
            f"WHERE {comp_filter} AND started_at IS NOT NULL AND completed_at IS NOT NULL {ds_filter}", params)
        if rows:
            durations_ms = []
            for r in rows:
                try:
...
def _tab_global_ai_llm(conn, schema, dataset_id) -> dict:
    ...
    if schema["pr_has_ai_summary"]:
        row = _one(conn,
            ...
            f"COUNT(*) as total FROM profiling_runs WHERE {comp_filter} {ds_pr}", pr_p)
    ...
    if schema["pr_has_ai_summary"]:
        row_sc = _one(conn,
            ...
            f"FROM profiling_runs WHERE {comp_filter} {ds_pr}", pr_p)
    ...
    return {
        ...
        "explainability": {
            ...
            "improvement": "If Hallucination Rate is 100%, the Azure LLM key is missing or the endpoint URL format is wrong.",
        },
    }
...
def _tab_profiling_ai(conn, schema, dataset_id) -> dict:
    ...
    comp_filter = schema["pr_completed_filter"]
    fail_filter = schema["pr_failed_filter"]

    row = _one(conn,
        f"SELECT COUNT(CASE WHEN {comp_filter} THEN 1 END) as comp, "
        f"COUNT(CASE WHEN {fail_filter} THEN 1 END) as fail, "
    ...
    if schema["pr_has_ai_summary"]:
        row_g = _one(conn,
            ...
            f"COUNT(*) as comp FROM profiling_runs WHERE {comp_filter} {ds_pr}", pr_p)
    ...
    return {
        "tab": "Profiling AI",
        "metrics": [
            M("profiling_success_rate", "Profiling Success Rate", psr_val, "%", psr_status,
              "completed_runs / total_runs × 100",
              {"completed": comp, "failed": fail, "total": total}),
            M("metadata_grounding_score", "Metadata Grounding Score", mg_val, "%", mg_status,
              "runs_with_non_empty_ai_summary / completed_runs × 100",
              {"grounded": grounded, "completed": comp2}),
            M("drift_detection_accuracy", "Drift Detection Coverage", dd_val, "%", dd_status,
              "profiling_runs_that_produced_drift_records / completed_runs × 100",
              {"runs_with_drift": runs_with_drift, "completed": comp}),
            M("avg_profiling_runtime_s", "Avg Profiling Runtime", dur["avg_s"], "s",
...
def _tab_dq_scores(conn, schema, dataset_id) -> dict:
    ...
    scores_rows = _all(conn,
        f"SELECT {score_col} FROM quality_snapshots WHERE {score_col} IS NOT NULL {ds_qs}" if score_col else
    ...
    if score_col and scores:
        recent_rows = _all(conn,
            f"SELECT {score_col} FROM quality_snapshots WHERE {score_col} IS NOT NULL {ds_qs} ORDER BY rowid DESC LIMIT 5", qs_p)
    ...
    return {
        "tab": "DQ Scores",
        "metrics": [
            M("health_score_accuracy", "Health Score Accuracy", hsa_val, "%", hsa_status,
              "quality_snapshots WHERE score BETWEEN 0 AND 100 / total_snapshots × 100",
              {"valid": valid_snaps, "total": total_snaps}),
            M("rule_compliance_accuracy", "Rule Pass Rate", rca_val, "%", rca_status,
              "passed_rule_results / total_rule_results × 100",
              {"passed": passed, "total": rr_total}),
            M("avg_health_score", "Avg Dataset Health Score", ahs_val, "%", ahs_status,
              f"mean({score_col or 'score'}) from quality_snapshots", {"snapshots": len(scores)}),
            M("health_degradation_velocity", "Health Degradation Velocity", velocity, "pts", hdv_status,
              "most_recent_score - oldest_score in last 5 snapshots", {"window": len(recent) if score_col and scores else 0}),
        ],
...
def _tab_dq_rules(conn, schema, dataset_id) -> dict:
    ...
    row_rules = _one(conn,
        f"SELECT COUNT(CASE WHEN {active_filter} THEN 1 END) as active, "
    ...
    return {
        "tab": "DQ Rules",
        "metrics": [
            M("rule_execution_success_rate", "Rule Execution Rate", resr_val, "%", resr_status,
              "active_rules_with_at_least_one_run / total_active_rules × 100",
              {"executed": executed, "active": active_rules, "total": total_rules}),
            M("rule_recommendation_acceptance_rate", "AI Rule Acceptance Rate", rrar_val, "%", rrar_status,
              "active_ai_suggested_rules / total_ai_suggested_rules × 100",
              {"accepted": ai_accepted, "suggested": ai_suggested}),
            M("hallucinated_rule_rate", "Hallucinated Rule Rate", hrr_val, "%", hrr_status,
              "ai_rules_with_zero_run_history / total_ai_rules × 100",
              {"never_run": never_run, "ai_rules": ai_suggested}),
            M("rule_coverage_rate", "Dataset Rule Coverage", rcr_val, "%", rcr_status,
...
def _tab_monitoring_trends(conn, schema, dataset_id) -> dict:
    ...
    if ts_col:
        row_7 = _one(conn,
            f"SELECT COUNT(*) as cnt FROM profiling_runs WHERE {ts_col} >= ? {ds_pr}",
            [cutoff_7d] + pr_p)
    ...
    return {
        ...
        "metrics": [
            M("avg_runs_last_7_days", "Profiling Activity (7-day)", avg_per_day, "runs/day", art_status,
              "total_profiling_runs in last 7 days / 7", {"runs_last_7d": runs_last_7}),
            M("drift_detection_precision", "Drift Alert Precision" if ddp_unit == "%" else "Total Drift Events",
              ddp_val, ddp_unit, ddp_status, ddp_formula,
              {"significant": significant, "total": drift_total, "severity_distribution": schema["severity_dist"]}),
            M("drift_volume_trend", "Drift Volume Trend (7-day vs prev 7-day)", trend, "%", dvt_status,
              "(last_7d_drift_count - prev_7d_drift_count) / prev_7d × 100", {"last_7d": last7, "prev_7d": prev7}),
            M("forecast_error_rate", "Health Score Volatility", volatility, "pts std", vol_status,
              "stdev(quality_snapshots.score)", {"samples": len(score_vals)}),
        ],
...
def _tab_anomalies_ai(conn, schema, dataset_id) -> dict:
    ...
    if ctx_col:
        row_ctx = _one(conn,
            f"SELECT COUNT(CASE WHEN {ctx_col} IS NOT NULL AND TRIM({ctx_col})!='' THEN 1 END) as has_ctx, "
            f"COUNT(*) as total FROM temporal_checks WHERE 1=1 {ds_tc}", tc_p)
    ...
    return {
        "tab": "Anomalies AI",
        "metrics": [
            M("anomaly_precision", "Temporal Check Anomaly Rate", ap_val, "%", ap_status,
              "temporal_checks WHERE LOWER(status)='open' / total × 100",
              {"open": open_cnt, "resolved": resolved_cnt, "total": tc_total}),
            M("anomaly_open_rate", "Open Anomaly Rate", ao_val, "%", ao_status,
              "open_temporal_checks / total_temporal_checks × 100", {"open": open_cnt, "total": tc_total}),
            M("checks_with_context", "Checks With Context", cwc_val, "%", cwc_status,
              f"temporal_checks WHERE {ctx_col or 'explanation'} IS NOT NULL AND LEN>0 / total × 100",
              {"with_context": has_ctx, "total": ctx_total, "context_column_used": ctx_col or "none found"}),
            M("auto_fix_success_rate", "Check Resolution Rate", afsr_val, "%", afsr_status,
...
def _tab_data_lineage(conn, schema, dataset_id) -> dict:
    ...
    row_prof = _one(conn,
        ...
        f"WHERE {comp_filter} {ds_pr}", [dataset_id] if dataset_id else [])
    ...
    return {
        ...
        "metrics": [
            M("lineage_coverage", "Lineage Coverage", lc_val, "%", lc_status,
              "datasets_with_lineage_edges / total_datasets × 100", {"mapped": mapped, "total": total_datasets, "edges": edges}),
            M("broken_edge_count", "Broken Lineage Edges", broken, "", bec_status,
              "COUNT(lineage_edges WHERE LOWER(status)='broken')", {"broken": broken, "total": le_total}),
            M("missed_dependency_rate", "Low-Confidence Dependency Rate", mdr_val, "%", mdr_status,
              "edges_with_confidence < 0.5 / total × 100", {"low_confidence": low_conf, "total": md_total}),
            M("datasets_profiled_rate", "Datasets Profiled", dpr_val, "%", dpr_status,
...
def _tab_knowledge_graph(conn, schema, dataset_id) -> dict:
    ...
    return {
        ...
        "metrics": [
            M("kg_relationship_precision", "Relationship Precision", rp_val, "%", rp_status,
              "KG_edges_with_confidence >= 0.7 / total_KG_edges × 100", {"high_confidence": high_conf, "total": kg_total}),
            M("kg_column_mapping_accuracy", "Column Relationship Coverage", kgcma_val, "%", kgcma_status,
              "column_relationship_edges / distinct_columns × 100", {"col_edges": col_edges, "distinct_cols": cp_distinct}),
            M("kg_hallucinated_relationship_rate", "Unscored Relationship Rate", kghrr_val, "%", kghrr_status,
              "edges_with_no_confidence_score / total × 100", {"unscored": unscored, "total": kg_total}),
        ],
...
def _tab_dq_assistant(conn, schema, dataset_id) -> dict:
    ...
    return {
        ...
        "metrics": [
            M("agent_routing_accuracy", "Notification Routing Accuracy", ara_val, "%", ara_status,
              "notifications_with_non_null_type / total_inbox × 100", {"tagged": tagged, "total": ni_total}),
            M("notification_content_rate", "Notification Content Rate", ncr_val, "%", ncr_status,
              "notifications_with_non_empty_message / total × 100", {"good": good_msgs, "total": msg_total}),
            M("action_agent_success_rate", "Governance Action Rate", aas_val, "%", aas_status,
...
def _tab_governance(conn, schema, dataset_id) -> dict:
    ...
    total_pol_actions = accepted + (pol_total - suggested)
    ...
    return {
        ...
        "metrics": [
            M("policy_adoption_rate", "Policy Adoption Rate", par_val, "%", par_status,
              "active_ai_policies / total_ai_suggested × 100",
              {"accepted": accepted, "suggested": suggested, "total_all_policies": pol_total}),
            M("classification_accuracy", "Column Sensitivity Classification", ca_val, "%", ca_status,
              "columns_with_sensitivity_label / total_columns × 100",
              {"classified": classified, "total": ca_total, "sensitivity_col": sens_col}),
            M("audit_log_completeness", "Audit Log Completeness", alc_val, "%", alc_status,
              "audit_log_entries / (rules_created + labels_applied + policy_actions) × 100",
              {"audit_entries": audit_entries, "expected_actions": expected_audit}),
        ],
...
def _tab_system_platform(conn, schema, dataset_id) -> dict:
    ...
    row_su = _one(conn,
        f"SELECT COUNT(CASE WHEN {comp_filter} THEN 1 END) as comp, "
        f"COUNT(CASE WHEN {fail_filter} THEN 1 END) as fail, "
        f"COUNT(*) as total FROM profiling_runs")
    ...
    return {
        "tab": "System / Platform",
        "metrics": [
            M("system_uptime", "System Uptime", su_val, "%", su_status,
              "completed_runs / (completed + failed) × 100", {"completed": comp, "failed": fail, "total": total}),
            M("api_throughput", "Processing Throughput", throughput, "runs/hr", at_status,
              "total_profiling_runs / elapsed_hours", {"total_runs": total}),
            M("avg_job_duration_ms", "Avg Job Duration", dur["avg_ms"], "ms",
              "healthy" if dur["avg_ms"] < 60000 else "warning",
              "mean(completed_at - started_at) in ms across completed runs", {"samples": dur["samples"]}),
        ],
...
def _tab_human_feedback(conn, schema, dataset_id) -> dict:
    ...
    return {
        "tab": "Human Feedback",
        "metrics": [
            M("ai_acceptance_rate", "AI Suggestion Acceptance Rate", aar_val, "%", aar_status,
              "active_policies / (active + dismissed) × 100",
              {"accepted": accepted, "dismissed": dismissed, "total": pol_total}),
            M("governance_activity_index", "Governance Activity (30-day)", governance_actions_30d, "actions", gai_status,
...
async def get_health_metrics(dataset_id: Optional[str] = Query(None)):
    ...
    try:
        ...
        tab_fns = [
            _tab_global_ai_llm, _tab_profiling_ai, _tab_dq_scores, _tab_dq_rules,
            _tab_monitoring_trends, _tab_anomalies_ai, _tab_data_lineage, _tab_knowledge_graph,
            _tab_dq_assistant, _tab_governance, _tab_system_platform, _tab_human_feedback,
        ]
        ...
        return {
            "generated_at": generated_at, "dataset_id": dataset_id, "db_path": DB_PATH,
            "schema_info": {
                ...
                "detected_pr_completed": schema["pr_completed_filter"],
                "detected_dq_active": schema["dq_active_filter"],
            },
            "tabs": tabs,
        }
...
async def debug_schema_inspection():
    """Debug endpoint to inspect actual database schema and values."""
    try:
        conn = _conn()
        schema = _introspect(conn)
