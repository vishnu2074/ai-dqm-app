...
def record_call(
    ...
    """
    Persists one LLM call to llm_usage_log and busts the dashboard cache
    so the new call shows up within the next 30s refresh.
    """
    ...
    try:
        ...
        conn.execute("""
            INSERT INTO llm_usage_log
              (called_at, model, prompt_tokens, completion_tokens, total_tokens,
               latency_ms, status, error_message, caller)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
        ...
        conn.commit()
        conn.close()
        _cache = None       # bust cache — next dashboard read gets fresh data
        _cache_ts = None
        logger.debug(
            f"LLM call recorded: {prompt_tokens}p + {completion_tokens}c = "
            f"{prompt_tokens + completion_tokens} tokens, {latency_ms:.0f}ms"
        )
...
def fetch_live(force: bool = False) -> Optional[dict]:
    ...
    try:
        ...
        # Resolve model name: from env, or most-used model in window, or alltime
        ...
        data = {
            ...
            "actual_cost":        None,   # no estimation — always from real data or null
            "cache_age_s":        0,
        }
